...
//
//         let signers = ReadDirStream::new(fs::read_dir(config.core.accounts_dir).await?)
//             .and_then(|e| fs::read(e.path()))
//             .err_into::<anyhow::Error>()
//             .and_then(|k| future::ready(SigningKey::from_bytes(&k).map_err(Into::into)))
//             .map_ok(Into::<Wallet<SigningKey>>::into)
//             .try_collect::<Vec<_>>()
//             .await?;
//         info!(
//             count = signers.len(),
...
//             .into_iter()
//             .map(|s| Account::new(requesting.clone(), s))
//             .collect::<FuturesUnordered<_>>()
//             .try_collect::<Accounts<_, _>>()
//             .await?;
...
//             requesting,
//             accounts,
//             monitors: [Box::new(pancake) as Box<dyn TxMonitor>].into(),
