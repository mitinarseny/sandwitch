...
mod multicall_inner;
use multicall_inner::multi_call::{
    Failed as FailedInner, MulticallCall as MultiCallCallInner,
    MulticallReturn as MutltiCallReturnInner,
};
...
impl MultiCallWrapper {
    ...
    fn encode_raw(self) -> MultiCallCallInner {
    ...
    fn decode_raw(call: MultiCallCallInner) -> Result<Self, InvalidData> {
        call.commands
            .into_iter()
            .zip(call.inputs)
...
impl AbiDecode for MultiCallWrapper {
    fn decode(bytes: impl AsRef<[u8]>) -> Result<Self, AbiError> {
        Self::decode_raw(MultiCallCallInner::decode(bytes)?)
...
impl MultiCallFailedWrapper {
    ...
    fn encode_raw(self) -> FailedInner {
        FailedInner {
            index: self.0.into(),
    ...
    fn decode_raw_as(call_types: Vec<CallType>, raw: FailedInner) -> Result<Self, InvalidData> {
    ...
    fn decode_as(call_types: Vec<CallType>, data: bytes::Bytes) -> Result<Self, InvalidData> {
        Self::decode_raw_as(call_types, FailedInner::decode(data)?)
    }
}
...
impl MultiCallOutputWrapper {
    ...
    fn encode_raw(self) -> MutltiCallReturnInner {
        let len = self.0.len();
    ...
    fn decode_raw_as(
        ...
        raw: MutltiCallReturnInner,
    ) -> Result<Self, InvalidData> {
        let MutltiCallReturnInner { successes, outputs } = raw;
    ...
    fn decode_as(call_types: Vec<CallType>, data: bytes::Bytes) -> Result<Self, InvalidData> {
        Self::decode_raw_as(call_types, MutltiCallReturnInner::decode(data)?)
    }
}
